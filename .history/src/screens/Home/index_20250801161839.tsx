...
import {setQueries} from '../../store/querySlice';
...
const Home: React.FC = (): JSX.Element => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!authToken) return;
      try {
        const [userRes, reviewRes, scanRes, workshopsRes, queriesRes] =
          await Promise.all([
            getUserDetails(authToken),
            getAllReviews(authToken),
            getAllScans(authToken),
            getAllWorkshops(authToken),
            getAllQueries(authToken),
          ]);
        ...
        if (workshopsRes?.success) {
          console.log('Response: All Workshops:', workshopsRes);
          dispatch(setWorkshops(workshopsRes.workshops));
        }
        if (queriesRes?.success) {
          console.log('Response: All Workshops:', queriesRes);
          dispatch(setQueries(queriesRes.queries));
        }
      } catch (error) {
      ...
      <View style={{flex: 1}}>
        ...
        <View style={styles.screen}>
          ...
          <View style={styles.lowerBody}>
            ...
            <OrangeButton title="Live Data" onPress={handleGoToLiveData} />
            <TouchableOpacity
