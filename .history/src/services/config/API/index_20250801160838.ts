...
const API_ENDPOINTS = {
  ...
  getAllWorkshops: '/user/get_all_workshops',
  submitWorkshopQuery: '/user/submit_workshop_query',
  getAllQueries: '/user/get_user_workshop_queries',
};
...
export type GetAllWorkshopsResponse = {
  message: string;
  workshops: any;
  success: boolean;
  error?: any;
...
export const getAllWorkshops = async (
  authToken: string | null,
): Promise<GetAllWorkshopsResponse | undefined> => {
  try {
    const headers = {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${authToken}`,
    ...
    const response = await axiosInstance.get<GetAllWorkshopsResponse>(
      API_ENDPOINTS.getAllWorkshops,
      {
        headers,
        validateStatus: status => status >= 200 && status < 500,
      },
    ...
    console.error('getAllWorkshops API error:', error.message || error);
    throw {success: false, message: error.message || 'Unknown error'};
  }
};

export type SubmitQueryBody = {
  workshop_id: number;
  full_name: string;
  email: string;
  phone_number: string;
...
export const submitWorkshopQuery = async (
  ...
  try {
    ...
    const response = await axiosInstance.post<SubmitQueryResponse>(
      API_ENDPOINTS.submitWorkshopQuery, // ← replace with actual endpoint like "/api/workshop-query"
      body,
      {
        headers,
        validateStatus: status => status >= 200 && status < 500,
    ...
    console.error('SubmitWorkshopQuery API error:', error.message || error);
    throw {success: false, message: error.message || 'Unknown error'};
  }
};
...
export type GetAllQueriesResponse = {
  queries: Query[];
  success: boolean;
  error?: any;
};
...
export const getAllQueries = async (
  ...
  try {
    ...
    const response = await axiosInstance.get<GetAllQueriesResponse>(
      API_ENDPOINTS.getAllQueries, // ← Replace with your actual endpoint like '/api/queries'
      {
        headers,
        validateStatus: status => status >= 200 && status < 500,
      },
    ...
    console.error('getAllQueries API error:', error.message || error);
    return {
      queries: [],
      success: false,
