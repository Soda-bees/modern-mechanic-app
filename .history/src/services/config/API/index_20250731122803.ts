...
export const getAllWorkshops = async (
  ...
  try {
    ...
    console.log('api log 1');

    const response = await axiosInstance.post<getAllWorkshopsResponse>(
    ...
    console.log('api log 2');

    return response.data;
  } catch (error: any) {
    console.error('Get all workshops API error:', error.message || error);
    throw {success: false, message: error.message || 'Unknown error'};
  }
};
