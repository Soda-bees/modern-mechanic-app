...
const AddUserVehicle: React.FC = (): JSX.Element => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log(body);
      // Call the signup API
      const response = await addVehicle(body, authToken);
      console.log('Add Vehicle Response:', response);
      if (response?.success) {
        dispatch(setUserData(response?.userData));
        Alert.alert('Success', 'Your vehicle is added successfully!', [
          {
            text: 'OK',
            onPress: () => {
              navigation.goBack();
      ...
      Alert.alert('Error', 'An error occurred while adding your vehicle.', [
        {
          text: 'OK',
          onPress: () => {
            navigation.goBack();
